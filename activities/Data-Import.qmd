...
The data was originally sourced from The Movie Database (TMDb) in accordance with their terms of use. And was collected by kaggle. This data was used to analyze which movies produced higher ratings, and higher commercial success.
\
\
\
\
...
```{r}
library(tidyverse)
imdb_messy <- read_csv("~/Documents/D.S. 112/mac-112-portfolio/data/imdb_5000_messy.csv")
```
...
1 movie had a lot of NA's which leads me to ask was it a real movie, because how is it not gonna be in color or black and white? Then there is no image at all.
#### Part b {.unnumbered}
...
There is multiple different catagories that classify the same thing.
\
\
\
\
...
So that what we do can be reproduced by others.
...
imdb_messy |> 
  summarize(num_na = sum(is.na(actor_1_facebook_likes)))
```
...
Forexample for actor 3 name, some movies might not have an actor 3. SO maybe some movies just dont have that specific variable. And those films are porbably the smaller films.
...
```
They are all documentaries. that makes sense.
\
\
\
\
...
We didnt. All we did is just ignored the NA's during the calculation.
#### Part b {.unnumbered}
...
We removed all movies with data that contains an NA. Its good for situation when we need to examine the complete data of every movie, but a bad idea when we dont wan tto do that.
#### Part c {.unnumbered}
