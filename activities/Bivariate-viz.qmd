...
# This has 2 variables in contrast to when we only had 1 variable
ggplot(elections, aes(y = repub_pct_20, x = repub_pct_16))
```

```{r eval = FALSE}
# Add a layer of points for each county
# Geom point makes a scatter plot
ggplot(elections, aes(y = repub_pct_20, x = repub_pct_16)) +
  geom_point()
```
...
# each different number has a really fun shape.
ggplot(elections, aes(y = repub_pct_20, x = repub_pct_16)) +
  geom_point(shape = 3)
```
...
# I thought fill would make the point orange. I was wrong its color.
ggplot(elections, aes(y = repub_pct_20, x = repub_pct_16)) +
...
# instead of geom point we need to use geom text and then let the text know what it needs to use
ggplot(elections, aes(y = repub_pct_20, x = repub_pct_16)) +
...
-   the strength of the relationship (weak/moderate/strong)\
 its a strong positive relationship. the higher the support was in 2016 the higher it was in 2020.
-   the direction of the relationship (positive/negative)\
positive
-   outliers (in what state do counties deviate from the national trend? Any ideas why this might be the case?)
TX a little bit because it kind of varies from the norm line at the beggining but i dont see any like serious outlier.
...
#maybe I am doing it wrong but I would say rent is slightly a better predicter as it kind of shows the higher the rent the less they voted for repub. (although i feel like neither were that great of a predictor compared to repub_pct_16)
```
...
ggplot(elections, aes(y = repub_pct_20, x = historical)) +
  geom_point()
```
This is not helpful because it is just three strait vertical lines and it is hard to see our points.
...
histroicaly red stated had higher repub county count.
...
```{r eval = FALSE}
# red is blue, purple is green, and blue is red
ggplot(elections, aes(x = repub_pct_20, fill = historical)) +
  geom_density()
```

```{r eval = FALSE}
# changes the colors manualy
ggplot(elections, aes(x = repub_pct_20, fill = historical)) +
  geom_density() +
...
```{r eval = FALSE}
# What does alpha = 0.5 do? It makes them slightly translucent.
...
# Why is this terrible? its hard to see all the values.
ggplot(elections, aes(x = repub_pct_20, fill = historical)) +
...
-   Which do you like best? I like box plots.
-   What is one pro of density plots relative to boxplots? shows every variable
-   What is one con of density plots relative to boxplots? I find it easier to read boxplots
...
Name one pro and one con of using the "proportional bar plot" instead of one of the other three options. We can see the percentage of voters that voted each way out of the total number of voters

#### Part b {.unnumbered}

What's your favorite bar plot from part and why?
 a side by side bar plot cause I feel like it is easier to read.
