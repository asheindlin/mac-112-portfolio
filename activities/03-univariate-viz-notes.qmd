...
I would want a bar-graph with the peak name being the x axis and the level of difficulty being the y axis. This allows for easy comparison of difficulty between different peaks.
b.  What about a visualization of the *quantitative* `elevation` variable?
I would want a line graph. with each peak getting a different line. Elevation being y axis and length being x axis. So you can easily visualize the ascent of the different hikes.
...
7 hikes fall into difficult. 11 hikes fall into easy. And 27 fall into moderate. The moderate hike is the most common, and then easy , and then hard.
...
By adding + geom we cant tell R what kind of graph we want.
-   What, in general, is the first argument of the `ggplot()` function?
It is telling r what is being plotted and from which survey.
-   What is the purpose of writing `x = rating`?
It tells r what the x axis is.
...
It adds a geometric layer of what kind of plot we want.

-   We added the bars using `geom_bar()`? Why "geom"?
That's the geometric layer that is added.
-   What does `labs()` stand for?

-   What's the difference between `color` and `fill`?
fill is the inside of the bar graph color is the edges of the bar.
### Part b {.unnumbered}
