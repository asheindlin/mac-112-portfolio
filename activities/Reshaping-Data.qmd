...
In this class we worked on the different ways to reshape data. Reshaping is necessary because sometimes you may need to resshape your data in order to use certain visualizations.
...
The units of measurment are participants.
...
`ggplot(sleep_wide, aes(y = reaction time, x =days of sleep resitriction, color = ___))`

Day number would be the column and the values in each column would be the reaction time.
...
If I were to compare reaction time to day I would use pivot wider to make day# the column, and reaction time the value of the column.
#### Part b {.unnumbered}
...
-   How did this impact how the values are recorded in the `day` column? The values in day transformed from catagorical to a numerical value.
...
There appears to be a postitive relationship between day and reaction time. The greater the value of day the greater the value of reaction_time. This is the same for everyone except for subject 332 whose reaction time value spikes towards the middle of the Day value. And then the reaction time value lessens.
